#!/usr/bin/env python3
"""
train_instr_v5.py — §16 instruction-level GNN: static analysis flags.

Requires preprocess_instr_v5.py to have been run first (data/*_instr_v5_graphs.pkl).

Architecture (adds static flag over §13):
  nn.Embedding(111, 128)              # opcode vocab -> dense embedding
  cat(embedding, const_mag, flag)     # (N, 130)  — flag is a float [0,1]
...
Changes vs train_instr_v2.py (§13):
  - conv1 input: embed_dim+1 -> embed_dim+2 (adds static flag channel)
  - Forward uses x[:,2] as the binary static analysis flag (float, not embedded)
  - Data files: *_instr_v5_graphs.pkl
  - Checkpoint default: model_instr_v5.pt

The flag is appended directly as a float (not looked up through an Embedding)
because it is binary and ordinal: 0.0 = clean, 1.0 = suspicious. No learned
representation is needed — the information is already maximally discrete.

Usage:
    python train_instr_v5.py --epochs 30 --hidden 64
"""
...
class InstructionGNN(nn.Module):
    def __init__(self, vocab: int = VOCAB_SIZE, embed_dim: int = 128, hidden: int = 64):
        ...
        # input: opcode_embed(128) + const_mag(1) + static_flag(1) = 130
    ...
    def forward(self, x, edge_index, edge_type, batch):
        ...
        static_flag = x[:, 2].unsqueeze(-1)   # 0.0 = clean, 1.0 = suspicious
        h = torch.cat([self.embed(opcode_ids), const_mag, static_flag], dim=-1)
...
def main():
    ...
    ap.add_argument("--checkpoint", type=str,   default="model_instr_v5.pt")
    args = ap.parse_args()
    ...
    for split in ["train", "valid", "test"]:
        if not (DATA / f"{split}_instr_v5_graphs.pkl").exists():
            print(f"Missing data/{split}_instr_v5_graphs.pkl -- run preprocess_instr_v5.py first.")
            sys.exit(1)

    print("Loading graphs ...")
    train_data = load_graphs(DATA / "train_instr_v5_graphs.pkl")
    valid_data = load_graphs(DATA / "valid_instr_v5_graphs.pkl")
    test_data  = load_graphs(DATA / "test_instr_v5_graphs.pkl")
    print(f"  train={len(train_data)}  valid={len(valid_data)}  test={len(test_data)}")

    # Report flag coverage so we know the patterns are firing
    flagged = sum(1 for d in train_data if d.x[:, 2].max().item() > 0.5)
    print(f"  train graphs with ≥1 flagged node: {flagged} / {len(train_data)} "
          f"({flagged/len(train_data)*100:.1f}%)")
    ...
    print(f"  §16 + static analysis flags (this run):                {test_acc:.2%}")

    delta = test_acc - 0.5875
    if delta > 0.005:
        print(f"\n  +{delta:.2%} over §13 — absence detection adds genuine signal")
    elif delta > -0.005:
        print(f"\n  Within noise of §13 — flags help but Devign noise limits gain")
    else:
        print(f"\n  Below §13 — check flag coverage stats above; patterns may be too sparse")
