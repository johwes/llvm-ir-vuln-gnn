#!/usr/bin/env python3
"""
preprocess_instr_v4.py — §15 instruction-level graphs: register name embedding.

Adds one improvement over preprocess_instr_v2.py (§13):

Name bucket feature (x[:,2]):
  At -O0, clang preserves source variable names in LLVM IR register names —
  %buf.addr, %size, %cmp, %ret, etc. These names carry semantic signal that
  opcode categories alone cannot express: a load from %size is semantically
  different from a load from %buf even if the opcode is identical.

  For each node, the register/argument name is normalized (strip .addr suffix,
  lowercase) and hashed to one of NAME_BUCKETS=64 buckets via FNV-1a. The
  bucket ID is stored as x[:,2] and looked up through a learned nn.Embedding
  in the model (train_instr_v4.py), giving a 16-dim name representation.

  Anonymous instructions (no name, or purely numeric SSA index) map to
  bucket 0 (reserved sentinel, no name signal).

No new compilation flags required — register names are present in standard
-O0 IR. Based on §13 (v2): 3 relations (CFG/DFG/Global), drops the state
edges from §14 which did not improve results.
...
Node feature shape: (N, 3) float32
  ...
  x[:,2] = name_bucket       (cast to long for nn.Embedding, 0 = anonymous)

Outputs: data/{train,valid,test}_instr_v4_graphs.pkl

Usage:
    python preprocess_instr_v4.py --subset 200 --skip-download
    python preprocess_instr_v4.py --skip-download
    python preprocess_instr_v4.py --workers 8 --skip-download
"""
...
# ---------------------------------------------------------------------------
# Opcode vocabulary (identical to v2/v3)
# ---------------------------------------------------------------------------
...
VOCAB_SIZE    = 111
NAME_BUCKETS  = 64   # hash space for register/variable names; 0 = anonymous
...
# ---------------------------------------------------------------------------
# Categorical call targets (identical to v2/v3)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Perfograph constant encoding (identical to v2/v3)
# ---------------------------------------------------------------------------

_CONST_INT_VAL_RE = re.compile(r'^i\d+\s+(-?\d+)$')
_CONST_FP_VAL_RE  = re.compile(r'^\S+\s+(.+)$')
_ANON_NAME_RE     = re.compile(r'^\d+$')
...
# ---------------------------------------------------------------------------
# §15: Name bucket — FNV-1a hash of register/variable name
# ---------------------------------------------------------------------------

def _name_bucket(name: str) -> int:
    """
    Map an LLVM IR register or argument name to a bucket in [1, NAME_BUCKETS].
    Anonymous values (empty string or purely numeric SSA index) return 0.

    Normalization: strip clang's .addr parameter-copy suffix, lowercase.
    At -O0, clang emits named allocas for all locals (%buf.addr, %size, %i)
    and preserves argument names (%buf, %n) — these contain the source
    variable names without requiring -g debug metadata.

    Uses FNV-1a 32-bit for deterministic, cross-platform hashing.
    """
    name = name.removesuffix(".addr").lower().strip()
    if not name or _ANON_NAME_RE.match(name):
        return 0
    h = 2166136261  # FNV offset basis
    for c in name:
        h ^= ord(c)
        h = (h * 16777619) & 0xFFFFFFFF  # FNV prime, 32-bit wrap
    return (h % NAME_BUCKETS) + 1  # [1, NAME_BUCKETS]
...
def ir_to_graph_instr(ir_text: str) -> dict | None:
    ...
    node_names:      list[int]   = []  # §15: name bucket [0, NAME_BUCKETS]
    ...
    for arg in target_fn.arguments:
        ...
        node_magnitudes.append(0.0)
        node_names.append(_name_bucket(arg.name))  # §15: argument name
        node_counter += 1
    ...
    for block in target_fn.blocks:
        ...
        for instr in block.instructions:
            ...
            node_magnitudes.append(0.0)
            node_names.append(_name_bucket(instr.name))  # §15: result name
            node_counter += 1
            ...
            for op in instr.operands:
                ...
                if vk == VK_INSTRUCTION or vk == VK_ARGUMENT:
                    ...
                    if opid not in constant_cache:
                        ...
                        node_names.append(0)  # constants have no name
                        node_counter += 1
                    ...
                    if name not in mock_cache:
                        ...
                        node_magnitudes.append(0.0)
                        # Global/function nodes: use the function name itself
                        node_names.append(_name_bucket(name))
                        node_counter += 1
    ...
    x          = np.column_stack([
                     ...
                     np.array(node_names,      dtype=np.float32),
                 ])
...
def main():
    ...
    for split in ["train", "valid", "test"]:
        ...
        dst = DATA / f"{split}_instr_v4_graphs.pkl"
        if not src.exists():
    ...
    print("\nDone. Run train_instr_v4.py next.\n")
