#!/usr/bin/env python3
"""
train_v2.py — §13 block-level GNN: Perfograph constant encoding (46 features).

Requires preprocess_v2.py to have been run first (data/*_v2_v2_graphs.pkl).
...
N_FEATURES = 46   # must match preprocess_v2.py: 3 structural + 7 opcode + 3 icmp + 2 type + 30 API + 1 const_log
...
def main():
    ...
    ap.add_argument("--checkpoint", type=str,   default="model_v2.pt",
    ...
    for split in ["train", "valid", "test"]:
        if not (DATA / f"{split}_v2_graphs.pkl").exists():
            print(f"Missing data/{split}_v2_graphs.pkl — run preprocess.py first.")
            sys.exit(1)

    print("Loading graphs ...")
    train_data = load_graphs(DATA / "train_v2_graphs.pkl")
    valid_data = load_graphs(DATA / "valid_v2_graphs.pkl")
    test_data  = load_graphs(DATA / "test_v2_graphs.pkl")
